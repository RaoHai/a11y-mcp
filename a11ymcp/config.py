"""Configuration module for a11ymcp.

This module provides access to user configuration stored in one of these locations:
1. $A11Y_MCP_CONFIG_DIR/a11ymcprc if $A11Y_MCP_CONFIG_DIR is defined
2. $XDG_CONFIG_HOME/a11ymcp/a11ymcprc if $XDG_CONFIG_HOME is defined
3. $HOME/.a11ymcprc
...
DEFAULT_CONFIG = {
    "logger": {
        ...
        "path": str(Path.home() / ".a11ymcp"),  # Default logger path
    },
    "files": {
...
def get_config_path() -> Path:
    ...
    1. $A11Y_MCP_CONFIG_DIR/a11ymcprc if $A11Y_MCP_CONFIG_DIR is defined
    2. $XDG_CONFIG_HOME/a11ymcp/a11ymcprc if $XDG_CONFIG_HOME is defined
    3. Fallback to $HOME/.a11ymcprc

    Returns:
        Path to the config file
    """
    # Check $A11Y_MCP_CONFIG_DIR first
    if "A11Y_MCP_CONFIG_DIR" in os.environ:
        path = Path(os.environ["A11Y_MCP_CONFIG_DIR"]) / "a11ymcprc"
        if path.exists():
            return path

    # Check $XDG_CONFIG_HOME next
    if "XDG_CONFIG_HOME" in os.environ:
        path = Path(os.environ["XDG_CONFIG_HOME"]) / "a11ymcp" / "a11ymcprc"
        if path.exists():
            return path

    # Fallback to $HOME/.a11ymcprc
    return Path.home() / ".a11ymcprc"
